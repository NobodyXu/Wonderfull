...
void loop()                                  // Main loop auto-repeats
{
  ...
  if((irLeft == 0) && (irRight == 0))        // If both sides detect
  {
    ...
    turnLeft(600);                           // Turn left about 90 degrees, 120 degrees = 800, 60 degrees = 400
  }
...
int irDetect(int irLedPin, int irReceiverPin, long frequency)
{
  ...
  delay(1);                                  // Wait 1 ms
  int ir lRead(irReceiverPin);       // IR receiver -> ir variable
  delay(1= digita);                                  // Down time before recheck
